...
Site.prototype.authorizationURL = function (redirectURI, scopes, state) {
    ...
    authUrl.query.client_id = this.options.client_id;
 ...
 * @param {function} cb    Callback, which takes err and a Token object
 */
Site.prototype.processAuthorizationResponse = function (params, cb) {
    if (params.error) {
    ...
    this.requestToken({ authCode: params.code }, function(err, tokens) {
        if (err) {
            return cb(err);
        }
        cb(null, new Api(tokens, this));
    });
};


Site.prototype.refreshAuthToken = function (token, cb) {
    if (typeof token === "object") {
        token = token.refresh_token;
    }
    this.requestToken({refreshToken: token}, function (err, tokens) {
        cb(err, tokens);
    });
};
