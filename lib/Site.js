 ...
 * @param {object} [options]    Extra options, currently only includeRefreshToken {bool}
 * @param {function} cb         Callback, which takes err, a Token object and an Api object
 */
Site.prototype.processAuthorizationResponse = function (params, redirectURI, state, options, cb) {
    ...
    this.requestToken({
        ...
        includeRefreshToken: options.includeRefreshToken !== false // default to true (backwards compatible)
    }, function(err, tokens) {
        if (err) {
            return cb(err);
        }
...
Site.prototype.requestToken = function (opts, cb) {
    ...
    if (opts.includeRefreshToken) {
        options.form.include_refresh_token = '1';
    }
