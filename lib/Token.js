 ...
 * @constructor
 */
let Token = function (tokenData, site, changeCallback) {
    try {
...
Token.prototype.setChangeCallback = function(callback) {
 ...
 * @param {callback} cb
 */
Token.prototype.revoke = function(cb) {
    if (!this.refresh_token) {
        return cb(new Error("There's no refresh token to revoke"));
    }
    let self = this;

    this.site.revokeRefreshToken(this.refresh_token, function(err) {
        if (err) {
            return cb(err);
        }
